...
from my_policy import PolicyGRU      # the RNN
...
inputs, targets, init_states, _, _ = task.generate(1, n_t) # get sample inputs so we know how many for policy
n_task_inputs = inputs['inputs'].shape[2]

# define the RNN
n_units = 256
policy = PolicyGRU(env.observation_space.shape[0] + n_task_inputs, n_units, env.n_muscles, device=device)
