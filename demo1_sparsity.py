...
from my_policy_sparsity import Policy # the RNN with sparsity structural constraint
...
# binary sparsity matrix
p = 0.4
S = th.bernoulli(th.full((n_units, n_units), p, device=device))

# define the policy
policy = Policy(env.observation_space.shape[0] + n_task_inputs, 
                n_units, env.n_muscles, device=device, sparsity=S)
...
for i in range(5):
    ...
    policy.apply_masks()                                         # enforce sparsity constraint after the update
